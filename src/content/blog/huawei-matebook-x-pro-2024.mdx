...
**UPDATE 27/07/2026**:
I managed to create a driver for the [camera](https://github.com/AlexDaichendt/gc2607-camera-linux) and [fingerprint sensor](https://github.com/AlexDaichendt/sil6250-linux).
Battery life is still not that great. Averaging around 4 hours with my Rust-heavy workflows. Bluetooth has been a bit better.
Overall, quite good experience now.
...
| Audio       | 8086:7e28                                   | :ok:               |
| Webcam      | 8086:7d19                                   | :white_check_mark: (*)               |
| Fingerprint | SIL6250                                     | :white_check_mark: (*)               |

(*) Requires out-of-tree driver installation 
...
CORRECTION 27/06/26: It is a GalaxyCore GC2607, producing a 1920x1080@29.90 video stream. Due to the nature of the Intel stack, a user space feeder program must run in the background. The quality of the camera is, for a Webcam, excellent. As far as I've used this camera, I am very content with it. 
...
CORRECTION 27/06/26: It was not Goodix. It is a Petaic Silead 6250 and thanks to the Clankers we got full driver support in Linux now.
The fingerprint sensor is a Match-On-Host one, records a 66x80 image and uses a SIFT-class verification algorithm. My matcher implementation is a reimplementation of what the Windows driver is doing and performs better overall (lower FRR) and is even tunable now that we have the source code. 
Having used the sensor for a few days, I'm quite happy with it. 
